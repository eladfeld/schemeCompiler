...
let rec collect_sexp expr =
  ...
  let rec build_const_tbl sexprs const_tbl current_offset=
    let string_to_asciis str =
      if String.length str = 0 then "\"\"" else
      String.concat ", " (List.map (fun asci -> string_of_int (Char.code asci)) (string_to_list str)) in
    let sexpr_to_const_entry sexpr const_tbl current_offset=
      ...
      | Sexpr(String(str)) -> (current_offset + 9 + (String.length str) ,(Sexpr(String(str)), (current_offset, "MAKE_LITERAL_STRING " ^ string_to_asciis str ^"\n") ) )
